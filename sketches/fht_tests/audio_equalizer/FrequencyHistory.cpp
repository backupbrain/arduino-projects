...
void FrequencyHistory::_initialize() {
  ...
  this->_exponentialToLinearFrequencyCoefficient = 0;
...
void FrequencyHistory::loadFromFHT(byte* frequencies) {
  ...
  for (int index = 0; index < this->_height; index++) {
    intensity = this->_scaleFrequencyIndex(frequencies, index, newScale);
...
/*
byte FrequencyHistory::_scaleFrequencyIndex(byte* frequencies, int frequencyIndex, int newScale) {
  if (frequencyIndex < 1) frequencyIndex = 1;
  int minFrequencyIndex = floor(newScale*(float)frequencyIndex/this->_fftResolution);
  if (minFrequencyIndex < 1) minFrequencyIndex = 1; // frequencies[0] is the total amplitude
  
  int maxFrequencyIndex = floor(newScale*(float)(frequencyIndex+1)/this->_fftResolution);
  if (maxFrequencyIndex > (this->_fftResolution-1)) maxFrequencyIndex = this->_fftResolution-1;

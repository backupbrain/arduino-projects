...
const byte AUDIO_SAMPLE_LENGTH = 128;
//char im[AUDIO_SAMPLE_LENGTH];
char data[AUDIO_SAMPLE_LENGTH]; 
char data_avgs[AUDIO_SAMPLE_LENGTH];
...
AudioSampler audioSampler = AudioSampler(mic, AUDIO_SAMPLE_RATE_HZ, AUDIO_SAMPLE_LENGTH); // sample input from the microphone
...
void triChn(int bias){//read low, mid, high chn
  ...
  analogWrite(ledB, 255-data_avgs[chnLow]);
  analogWrite(ledG, 255-data_avgs[chnMid]-(bias/2));
  analogWrite(ledR, 255-data_avgs[chnHigh]-bias);
}
...
void grpChnLowPass(){//grp low pas 32 out of 64 bins into grp of low mid high, avg out in grp
  ...
  //output to rgb
  analogWrite(ledB, 255-tempLow);
  analogWrite(ledG, 255-tempMid);
  analogWrite(ledR, 255-tempHigh);
...
void treshChn(){//on if over threshold low, mid, high
  ...
  if(tempLow>150){
    ...
    analogWrite(ledR, 255-255);
    //delay(100);
  }

  else if (tempMid>150){
    //more mid gives green
    analogWrite(ledB, 255-255);
    analogWrite(ledG, 255-tempMid);
    analogWrite(ledR, 255-255);
    //delay(100);
  }
     ...
     */
    analogWrite(ledB, 255-tempLow-70);
    analogWrite(ledG, 255-tempMid-100);
    analogWrite(ledR, 255-tempHigh-150);
    //delay(100);
  }  

...
void loop() {
  ...
  char intensity = 0; // peak-to-peak level

  unsigned int signalMax = 0;
  unsigned int signalMin = 255;

  for (int i = 0; i < AUDIO_SAMPLE_LENGTH; i++) {
    if (samples[i] < 255) {
      if (samples[i] > signalMax) {
        signalMax = samples[i]; // save just the max levels
      } else {
        signalMin = samples[i]; // save just the min levels
      }
    }
    rgbLED._pwm(rgbColor.r, rgbColor.g, rgbColor.b, i, i + 1);
  }

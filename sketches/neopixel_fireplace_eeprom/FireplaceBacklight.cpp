...
void FireplaceBacklight::_initialize() {
  this->_lastFrameUpdate_ms = millis();
 ...
 */
void FireplaceBacklight::updateFrame(unsigned int timeDifference) { 
  /*
  this->_lastFrameUpdate_ms = this->_currentFrameUpdate_ms;
  this->_currentFrameUpdate_ms = millis();
  
  int timeDifference = this->_currentFrameUpdate_ms - this->_lastFrameUpdate_ms;
  /* */
...
SimpleNeopixel* FireplaceBacklight::getPixelStrip() {
  return this->_pixelStrip;
}

...
const int NEOPIXEL_GRID_HEIGHT = 8;
const int MAX_CHARS = 65;              // Max size of the input command buffer

const int FADE_LED_PIN = 9;
const int BEAT_LED_PIN = 11;
...
int sampleCounter = 0;
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(NEO_PIXEL_COUNT*8, NEO_PIXEL_PIN, NEO_GRB + NEO_KHZ800);
...
float beatSensitivity = 1.3; // fraction of average intensity;

const int INTENSITY_WINDOW_SIZE = 100;
float averageIntensities[INTENSITY_WINDOW_SIZE];
...
void loop() {
  ...
  colorWipe(pixels.Color(255, 0, 0), 50); // Redeoau
...
void spectrumLoop() {
  ...
  for (int i = 0; i < NEO_PIXEL_COUNT; ++i) {
    ...
    int allPixels = 8;
    int numPixelsTall = intensity * 8;
    
    unsigned int pixelPosition = 0;
    for (int y=0; y<allPixels; y++) {
      pixelPosition = (y*8)+i;
      if (y <= numPixelsTall) {
        pixels.setPixelColor(pixelPosition, pixelHSVtoRGBColor(hues[i], 1.0, intensity));
      } else {
  ...
  // average the intensities and push them onto the array
  averageIntensity = totalIntensities / NEO_PIXEL_COUNT;
  arrayPush(averageIntensity, averageIntensities, INTENSITY_WINDOW_SIZE);
  
  // go through average intensities and see if this one stands out
  float sampleAverage = 0;
  float averagesTotal = 0;
  for (int i=0; i<INTENSITY_WINDOW_SIZE; i++) {
    averagesTotal += averageIntensities[i];
  }
  sampleAverage = averagesTotal/INTENSITY_WINDOW_SIZE;
  
  if (averageIntensity > (sampleAverage * beatSensitivity)) {
    // beat!
    beatOn();
  } else {
    // no beat!
...
/**
 * push a new item onto the array, dropping the first item off
 * @param item the item to add to the array
 * @param items the array
 * @itemsLength the length of the array
 */
void arrayPush(float item, float* items, int itemsLength) {
  // push everything back one spot in the array
  for (int i=1; i<itemsLength; i++) {
    items[i-1] = items[i];
  }
  // put the item at the end of the items array
  items[itemsLength-1] = item;
}

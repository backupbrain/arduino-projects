...
// Approximating function approximates a function using a table
// and linearly interpolating for values "between" table entries.
//
// Example:
//   float MappingFunction(float raw_val) {
...
//
//     float approx_val = Interp(raw_val, kInterpData, n);
//     return approx_val;
//   }
...
inline ValT Interp(const KeyT& k, const InterpData<KeyT, ValT>* array, const int n) {
  if (n < 1) {
    Serial.print(__FUNCTION__); Serial.println(": unexpected error - array size empty");
    return ValT(0);
  }

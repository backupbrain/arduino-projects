...
WaterfallPlotter::WaterfallPlotter(unsigned int width, unsigned int height, uint8_t neopixelPin, byte* imageData) {
  this->_maxValue = 1.0;
  this->_width = width;
  this->_height = height;
  ...
  this->_pixelStrip = (Adafruit_NeoPixel*)malloc((height)*sizeof(Adafruit_NeoPixel));
  
  this->_pixelStrip = new Adafruit_NeoPixel(this->_width*this->_height, this->_neopixelPin, NEO_GRB + NEO_KHZ800);
  this->_pixelStrip->begin();
  
  this->_image = (Image*)malloc(sizeof(Image));
  this->_image = new Image(this->_width, this->_height, imageData);
...
void WaterfallPlotter::display() {
  ...
  for (int pixel = 0; pixel < maxPixels; pixel++) {
    ...
    this->_pixelStrip->setPixelColor(pixel, pixelColors[pixel].r, pixelColors[pixel].g, pixelColors[pixel].b);

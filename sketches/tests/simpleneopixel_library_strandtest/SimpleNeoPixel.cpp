...
SimpleNeopixel::SimpleNeopixel(unsigned int numPixels, unsigned char pin, unsigned char colorOrder) {
  this->_pin = pin;
  this->_numPixels = numPixels;
  this->_colorOrder = colorOrder;
  
  this->_PIXEL_PORT = PORTB;
  this->_PIXEL_DDR = DDRB;
  this->_PIXEL_BIT = 5;
...
void SimpleNeopixel::_sendBit( bool bitVal ) {
    //byte PIXEL_PORT = this->_PIXEL_PORT;
    //byte PIXEL_BIT = this->_PIXEL_BIT;
    if (  bitVal ) {				// 0 bit
